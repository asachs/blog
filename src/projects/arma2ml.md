...
Then you would run `ArmA2ML.exe -y -newest -file="path/to/file"` which would use the options that appeared in the file in the place of -mod command line options. In effect it would be the same as running `ArmA2ML.exe -y -newest -mod @ACE -mod @CBA -mod @CSM`. This makes editing large mod lists a bit easier.
